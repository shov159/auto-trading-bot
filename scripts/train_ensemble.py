...
def fetch_data(ticker="SPY", start="2020-01-01", end="2023-01-01"):
    ...
    # YFinance returns MultiIndex if multi tickers, but single here.
...
def train_ensemble():
    ...
    # 3. Environment Setup
    # stable-baselines3 requires vectorized environments for some algos
    env = DummyVecEnv([lambda: AdvancedTradingEnv(processed_df)])
    
    # 4. Train PPO (The Anchor)
    print("\n--- Training PPO (Trend Follower) ---")
    ...
    # DDPG needs action noise for exploration in continuous space
    n_actions = env.action_space.shape[-1]
    action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions))
    
    ddpg_model = DDPG("MlpPolicy", env, action_noise=action_noise, verbose=1, tensorboard_log=LOG_DIR)
    ddpg_model.learn(total_timesteps=30000) # DDPG is slower/sample efficient
